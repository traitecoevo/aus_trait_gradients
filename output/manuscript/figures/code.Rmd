...
ggpairs(for_corrplot,columns = (2:9), upper = list(continuous = wrap("cor", stars = FALSE)), columnLabels = c("Delta**{13}*C","MH","LA", "LMA","SM","N[area]", "SA:LA","WD"),
...
woody_field_traits_georef_mean_climate_wide %>%
  ...
  # mutate(na_col = rep(NA, 2927)) %>%
    # mutate(na_col2 = rep(NA, 2927)) %>%
  dplyr::select(log_leaf_capital_delta13C, log_height, log_leaf_area, log_LMA, log_seed_mass, log_leaf_N_calc) -> for_corrplot
ggpairs(for_corrplot, upper = list(continuous = wrap("cor", stars = FALSE)), columnLabels = c("Delta**{13}*C",  "MH","LA", "LMA", "SM", "N[area]"),
  labeller = "label_parsed",
