load_worldclim <- function(directory, name_of_csv) {
  ...
  if (length(paths_csv) == 0){
    ...
    extract_by_layer <- function(layer){
    i = layer
    ...
    map(.x = seq(length(names(worldclim))), .f=extract_by_layer) %>%
    ...
    map(.x = seq(length(names(worldclim))), .f=iterate_buffer) %>%
