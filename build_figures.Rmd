...
# au_bioclim[[5]][au_bioclim[[5]] < 0] <- NA
# raster::plot(au_bioclim[[5]], col=rev(terrain.colors(1000)))
# points(au_sites_clim$lat~au_sites_clim$lon, pch=19, col=rev(terrain.colors(1000))[round((au_sites_clim$Photosynthetically.active.radiation/max(au_sites_clim$Photosynthetically.active.radiation, na.rm=T)*1000-656)*1000/343,0)])
# 
# pdf("output/test.pdf", height=8, width=10)
...
austraits$traits %>% 
  ...
  summarise(value = mean(value)) %>%
  ungroup() %>%
  spread(key = trait_name, value = value) -> asd
...
austraits$traits %>% 
  ...
  ungroup() %>%
  spread(key = trait_name, value = value)%>%
  mutate(ci_over_ca_calc = (((-8-leaf_delta13C/(1+leaf_delta13C/1000))-4.4)/(27-4.4)))%>%
...
austraits$traits %>% filter(trait_name %in% c("woodiness","plant_growth_form")) %>%
  distinct(taxon_name) -> have_woody_data
  
austraits$traits %>% 
  filter(!taxon_name %in% have_woody_data$taxon_name) %>%
  pull(taxon_name) -> species_without_woody_data_3.0

updated_woody_data<-read_csv("data/no_woody_data.csv") 

austraits$taxa %>%
  filter(taxon_name %in% species_without_woody_data_3.0) %>%
  select(taxon_name, family) %>%
  filter(!taxon_name %in% updated_woody_data$taxon_name)


austraits$taxa %>%
  filter(taxon_name %in% species_without_woody_data_3.0) %>%
  select(taxon_name, family) %>%
  write_csv(., "output/no_woody_data_3.0.csv")

  read_csv("data/no_woody_data.csv") -> asd
...
woody_field_traits_georef %>%
  filter(trait_name %in% core_traits) %>%
  ggplot(aes(DNI, value)) + geom_point(alpha = 0.2, stroke=0)+ scale_y_log10() + facet_wrap(~trait_name, scales = "free_y") + xlab("Annual mean direct normal irradiation")
```

```{r, fig.width=10}
woody_field_traits_georef %>%
  filter(trait_name %in% core_traits) %>%
  ggplot(aes(Photosynthetically.active.radiation.1, value)) + geom_point(alpha = 0.2, stroke=0)+ scale_y_log10() + facet_wrap(~trait_name, scales = "free_y") + xlab("Average PAR January")
```

```{r, fig.width=10}
woody_field_traits_georef %>%
  filter(trait_name %in% core_traits) %>%
  ggplot(aes(annual_PAR, value)) + geom_point(alpha = 0.2, stroke=0)+ scale_y_log10() + xlab("Annual mean of monthly average PAR") + facet_wrap(~trait_name, scales = "free_y")
```
