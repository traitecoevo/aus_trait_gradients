...
n_woody_traits_site_summary<-woody_traits_site_summary %>% 
  group_by(dataset_id, latitude, longitude, Temp, Prec) %>% 
  summarise(n_traits=n_distinct(trait_name))

n_woody_traits_georef_site_summary<-woody_traits_georef %>% 
  group_by(dataset_id, latitude, longitude, Temp, Prec) %>% 
  summarise(n_traits=n_distinct(trait_name))
...
hist(n_woody_traits_site_summary$n_traits, xlab="Number of traits", ylab = "Sites (woody taxa only)", main = "Number of sites (total)")
hist(n_woody_traits_georef_site_summary$n_traits, xlab="Number of traits", ylab = "Sites (woody taxa only)", main = "Number of sites (georeferenced)")
...
woody_traits_georef_spread <- woody_traits_georef %>% 
  select(observation_id,Temp, Prec, taxon_name, trait_name, value) %>% 
  pivot_wider(names_from = "trait_name", values_from = "value")%>%
  unnest(trait_names[-12]) %>%
  mutate(Temperature_binary = ifelse(Temp > median(Temp, na.rm=T), "Above 17.5 C", "Below 17.5 C"))%>%
  mutate(Precipitation_binary = ifelse(Prec > median(Prec, na.rm=T), "Above 816 mm", "Below 816 mm"))
```
