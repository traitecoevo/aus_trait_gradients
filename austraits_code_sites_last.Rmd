...
#   
# austraits$traits %>%
#   filter(trait_name == "plant_photosynthetic_organ") %>%
#   group_by(taxon_name) %>%
#   mutate(num_types = n_distinct(value)) %>% 
#   filter(num_types == 1) %>% 
#   filter(value != "leaf") %>%
#   distinct(taxon_name) %>%
#   mutate(phyllode = "phyllode") -> phyllode_taxa

field_traits_leaf_area %>%
  left_join(compound_taxa) %>% 
  # filter((compound_species == "compound_present" & leaf_unit_measured %in% c(NA, "leaf"))) %>% filter(is.na(compoundness)) %>% distinct(taxon_name)
  # left_join(phyllode_taxa) %>% 
