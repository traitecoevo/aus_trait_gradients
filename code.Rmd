...
Load in trait data from Austraits

```{r, warning=FALSE, message=FALSE}
austraits <- readRDS("data/austraits/austraits-4.1.0.rds")
```

Filter Austraits by the focal traits including some accessory traits like "leaflet_area" and "leaf_N_per_dry_mass" which will eventually become "leaf_area" and "leaf_N_per_area" at a later point and convert to numeric values.
...
Filter as above. Cheesman_2020 is the only noted field, field experiment
...
We also remove all literature based values
...
Read in the now filled-in "data/leaf_compound_filter/datasets_with_leaf_area_edited.csv".
...
Now onto the woodiness categories. We use the woodiness detailed trait collated by Wenk_2022 for most taxa in Austraits, to determine woody or non-woody taxa.
...
woody_field_traits %>%
  ...
  mutate(value = as.numeric(value)) %>%
  group_by(observation_id, taxon_name, trait_name, dataset_id, growth_form_data, location_id)%>% 
  nest(.key = "value") %>%
...
```{r}
error_check(woody_field_traits, "leaf_N_per_area") -> error_check_leaf_N
```
