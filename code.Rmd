...
Convert $\delta^{13}C$ values to $\Delta^{13}C$ values.
...
png("output/manuscript/supps_figures/overall_distribution.png", height = 2000, width = 2000, res = 300)
overall_distribution +
  labs(title = "All trait data")
dev.off()

png("output/manuscript/supps_figures/woody_distribution.png", height = 2000, width = 2000, res = 300)
woody_distribution +
  labs(title = "Woody trait data")
dev.off()

png("output/manuscript/supps_figures/non_woody_distribution.png", height = 2000, width = 2000, res = 300)
non_woody_distribution +
  labs(title = "Non-woody trait data")
dev.off()

png("output/manuscript/supps_figures/overall_distribution_dataset.png", height = 2000, width = 4000, res = 200)
overall_distribution_dataset +
  labs(title = "All trait data")
dev.off()

png("output/manuscript/supps_figures/woody_distribution_dataset.png", height = 2000, width = 4000, res = 200)
woody_distribution_dataset +
  labs(title = "Woody trait data")
dev.off()

png("output/manuscript/supps_figures/non_woody_distribution_dataset.png", height = 2000, width = 3000, res = 200)
...
woody_field_traits_georef_mean_climate_wide %>%
  ...
  dplyr::select(growth_form, log_leaf_area, log_LMA, log_huber, log_seed_mass, log_leaf_N_calc, log_leaf_capital_delta13C, log_wood_dens, log_height) -> for_corrplot
ggpairs(for_corrplot,columns = 2:9, upper = list(continuous = wrap("cor", stars = FALSE)), columnLabels = c("LA", "LMA", "SA:LA","SM","N[area]","Delta**{13}*C","WD","MH"),
...
ggpairs_modified(for_corrplot, upper = list(continuous = wrap("cor", stars = FALSE))
...
woody_field_traits_georef_mean_climate_wide %>%
  ...
  # mutate(na_col = rep(NA, 2927)) %>%
    # mutate(na_col2 = rep(NA, 2927)) %>%
  dplyr::select(log_leaf_capital_delta13C, log_height, log_leaf_area, log_LMA, log_leaf_N_calc, log_seed_mass) -> for_corrplot
ggpairs(for_corrplot, upper = list(continuous = wrap("cor", stars = FALSE)), columnLabels = c("Delta**{13}*C",  "MH","LA", "LMA", "N[area]", "SM"),
  labeller = "label_parsed",
...
woody_field_traits_georef_mean_climate_wide %>%
  ...
  drop_na(growth_form) %>%
  # drop_na(AI_thorn) %>%
  dplyr::select(log_leaf_capital_delta13C, log_height, log_leaf_N_calc, log_LMA, log_seed_mass, log_leaf_area, Temp,log_Prec, log_MI, log_VPD, log_Prec_CV, log_prec_wq, sqrt_prec_dq, log_prec_hq, sqrt_prec_cq, cell) %>%
  ...
  pivot_longer(cols = c(log_Prec, log_VPD,log_Prec_CV, log_prec_wq, sqrt_prec_dq,log_MI,Temp,log_prec_hq,sqrt_prec_cq), names_to = "env_name", values_to = "env_value") %>% 
...
woody_field_traits_georef_mean_climate_wide %>%
  ...
  dplyr::select(log_leaf_capital_delta13C, log_height, log_leaf_N_calc, log_LMA, log_seed_mass, log_leaf_area,
                        log_huber,
                        log_wood_dens, Temp, log_Prec, log_VPD,log_Prec_CV, log_prec_wq, sqrt_prec_dq,log_prec_hq,log_MI,sqrt_prec_cq, cell) %>%
  pivot_longer(cols = c(log_leaf_capital_delta13C, 
                        log_leaf_area,
                        log_seed_mass,
                        log_LMA,
                        ...
                        log_wood_dens),
                        names_to = "trait_name", values_to = "trait_value") %>%
  pivot_longer(cols = c(log_Prec, log_VPD,log_Prec_CV, log_prec_wq, sqrt_prec_dq,log_prec_hq,log_MI,sqrt_prec_cq, Temp), names_to = "env_name", values_to = "env_value") %>% 
  group_by(trait_name, env_name) %>%
  nest() %>%
  ungroup %>%
  mutate(r2 = pmap(., fit_models)) %>%
...
print(xtable(fit_quad_woody %>%
  ...
  rename("log(MAP)" = "log_Prec",
         ...
         "log(Warmest quarter precipitation)" = "log_prec_hq",
         "MAT" = "Temp",
         "log(Moisture index)" = "log_MI")%>% pivot_longer(cols = -(trait_name)) %>% pivot_wider(names_from = "trait_name", values_from = "value")), include.rownames=FALSE, file = "output/manuscript/supps_tables/fit_quad_woody_prec.tex")
```
...
print(xtable(fit_quad_non_woody %>%
  ...
  rename("log(MAP)" = "log_Prec",
         ...
         "log(Warmest quarter precipitation)" = "log_prec_hq",
         "MAT" = "Temp",
         "log(Moisture index)" = "log_MI")%>% pivot_longer(cols = -(trait_name)) %>% pivot_wider(names_from = "trait_name", values_from = "value")), include.rownames=FALSE, file = "output/manuscript/supps_tables/fit_quad_non_woody_prec.tex")
```
...
out_interaction_woody %>%
  ...
  mutate(direction = map_chr(r2, ~.x[[4]])) %>%
...
```{r}
#no need to include leaf area N (without claculated values)
...
<!--   mutate(growth_form = if_else(woody_like_taxa == "Woody" | growth_form == "Woody", "Woody","Non-woody")) %>% -->
<!--   filter(!(trait_name %in% c("huber_value","wood_density") & growth_form == "Non-woody" )) %>%  -->
<!--     dplyr::select(cell, trait_name, value, prec, growth_form)%>% -->
<!--   group_by(cell, trait_name, growth_form, prec) %>% -->
<!--   filter(trait_name == "leaf_mass_per_area") %>% -->
<!--   mutate(value = log(value)) %>% -->
<!--   mutate(prec = log(prec)) %>% -->
<!--   filter(growth_form == "Woody") %>% -->
<!--   summarise(value = mean(value, na.rm = T)) %>% -->
<!--   ungroup() %>% -->
<!--   mutate(cor = cor(value, prec, use = "pairwise.complete.obs")) -->
...
<!--   data_for_plot$trait_name <- factor(data_for_plot$trait_name,  -->
<!--                                      levels  = c("leaf_capital_delta13C","plant_height","huber_value","leaf_area","leaf_mass_per_area","leaf_N_per_area_calc","seed_dry_mass","wood_density"), -->
<!--                                      labels = c("Delta^13~C~(`\u2030`)", "H~(m)", "SA:LA","LA~(mm^2)", "LMA~(g~m^{-2})", "N[area]~(g~m^{-2})", "SM~(mg)", "Wood~density~(mg~mm^{-3})")) -->
...
<!--   mutate(growth_form = if_else(woody_like_taxa == "Woody" | growth_form == "Woody", "Woody","Non-woody")) %>% -->
<!--   filter(!(trait_name %in% c("huber_value","wood_density") & growth_form == "Non-woody" )) %>%  -->
<!--     dplyr::select(taxon_name, trait_name, value, prec, growth_form)%>% -->
<!--   filter(trait_name == "leaf_mass_per_area") %>% -->
<!--   filter(growth_form == "Woody") %>% -->
<!--   group_by(taxon_name) %>% -->
<!--   mutate(n = length(taxon_name)) %>% -->
<!--   filter(n > 15) %>% -->
<!--   ungroup() %>% -->
<!--     drop_na(value, prec) %>% -->
<!--   ggplot(aes(x = prec, y = value)) + -->
<!--   geom_point() + -->
<!--     geom_smooth(method = "lm", aes(group = taxon_name)) + -->
<!--   scale_y_log10() + -->
<!--   scale_x_log10()  -->
